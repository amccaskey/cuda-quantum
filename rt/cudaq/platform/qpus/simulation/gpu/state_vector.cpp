...
state_vector::state_vector(const heterogeneous_map &config)
    : pImpl(std::make_unique<Impl>()), qpu(config) {}

state_vector::~state_vector() = default;

void state_vector::configure(const heterogeneous_map &config) {
  // nothing to do yet
}
