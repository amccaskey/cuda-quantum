...
#include "cudaq/simulators/Gates.h"
#include "cudaq/utils/Timing.h"
...
class CuStateVecCircuitSimulator
    : public cudaq::CircuitSimulatorBase<
          ScalarType, CuStateVecCircuitSimulator<ScalarType>> {
public:
  static std::string GetSimulatorName() {
    return std::is_same_v<ScalarType, double> ? "custatevec-fp64"
                                              : "custatevec-fp32";
  }
  static std::unique_ptr<cudaq::CircuitSimulator> create() {
    return std::make_unique<CuStateVecCircuitSimulator<ScalarType>>();
  }
  std::string name() const override { return GetSimulatorName(); }

  using Base = CuStateVecCircuitSimulator<ScalarType>;
...
protected:
  ...
  using cudaq::CircuitSimulatorBase<ScalarType, Base>::tracker;
  using cudaq::CircuitSimulatorBase<ScalarType, Base>::nQubitsAllocated;
  using cudaq::CircuitSimulatorBase<ScalarType, Base>::stateDimension;
  using cudaq::CircuitSimulatorBase<ScalarType, Base>::calculateStateDim;
  using cudaq::CircuitSimulatorBase<ScalarType, Base>::executionContext;
  using cudaq::CircuitSimulatorBase<ScalarType, Base>::gateToString;
  using cudaq::CircuitSimulatorBase<ScalarType, Base>::x;
  using cudaq::CircuitSimulatorBase<ScalarType, Base>::flushGateQueue;
  using cudaq::CircuitSimulatorBase<ScalarType, Base>::previousStateDimension;
  using cudaq::CircuitSimulatorBase<ScalarType,
                                    Base>::shouldObserveFromSampling;
  using cudaq::CircuitSimulatorBase<ScalarType, Base>::summaryData;
  ...
  void applyGate(const typename cudaq::CircuitSimulatorBase<
                 ScalarType, Base>::GateApplicationTask &task) override {
    ...
    if (task.operationName == "rx") {
      oneQubitOneParamApply<cudaq::rx<ScalarType>>(
          task.parameters[0], task.controls, task.targets[0]);
    } else if (task.operationName == "ry") {
      oneQubitOneParamApply<cudaq::ry<ScalarType>>(
          task.parameters[0], task.controls, task.targets[0]);
    } else if (task.operationName == "rz") {
      oneQubitOneParamApply<cudaq::rz<ScalarType>>(
          task.parameters[0], task.controls, task.targets[0]);
    } else {
...
CUDAQ_SIMULATOR_INITIALIZER(CuStateVecCircuitSimulator<double>)
#endif
