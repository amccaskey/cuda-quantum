...
class QppCircuitSimulator
    : public CircuitSimulatorBase<double, QppCircuitSimulator<StateType>> {
public:

  CUDAQ_REGISTER_SIMULATOR(QppCircuitSimulator<StateType>, qpp)
...
protected:
  ...
  std::size_t convertQubitIndex(std::size_t qubitIndex) {
    assert(this->stateDimension > 0 && "The state is empty, and thus has no qubits");
    return std::log2(this->stateDimension) - qubitIndex - 1;
  }
  ...
  double calculateExpectationValue(const std::vector<std::size_t> &qubits) {
    ...
    if constexpr (std::is_same_v<StateType, qpp::ket>) {
      result.resize(this->stateDimension);
#ifdef CUDAQ_HAS_OPENMP
#pragma omp parallel for
#endif
      for (std::size_t i = 0; i < this->stateDimension; ++i)
...
#pragma omp parallel for
  ...
  void addQubitsToState(std::size_t qubitCount,
    ...
    if (state.size() == 0) {
      ...
      if (stateData == nullptr) {
        state = qpp::ket::Zero(this->stateDimension);
        state(0) = 1.0;
      } else
        state = qpp::ket::Map(stateData, this->stateDimension);
      return;
    }
  ...
  using BaseT = CircuitSimulatorBase<double, QppCircuitSimulator<StateType>>;
  void applyGate(const BaseT::GateApplicationTask &task) override {
...
public:
  ...
  bool canHandleObserve() override {
    ...
    if (this->executionContext &&
        this->executionContext->shots != static_cast<std::size_t>(-1)) {
      return false;
    }

    return !this->shouldObserveFromSampling();
  }
...
CUDAQ_SIMULATOR_INITIALIZER(nvqir::QppCircuitSimulator<qpp::ket>)

// template <>
// const bool cudaq::CircuitSimulatorBase<
//     double, nvqir::QppCircuitSimulator<qpp::ket>>::registered_ =
//     cudaq::CircuitSimulatorBase<
//         double, nvqir::QppCircuitSimulator<qpp::ket>>::register_type();
