...
getGateByName(GateName name, const std::vector<Scalar> angles = {}) {
  ...
  case (GateName::U2): {
    ...
    return {{oneOverSqrt2, 0.},
            -oneOverSqrt2 * std::exp(lambda * cudaq::im<Scalar>),
            oneOverSqrt2 * std::exp(cudaq::im<Scalar> * phi),
            oneOverSqrt2 * std::exp(cudaq::im<Scalar> * (phi + lambda))};
  }
  case (GateName::U3): {
    auto theta = angles[0];
    auto phi = angles[1];
    ...
    return {{std::cos(theta / 2), 0.},
            -std::exp(cudaq::im<Scalar> * lambda) * std::sin(theta / 2),
            std::exp(cudaq::im<Scalar> * phi) * std::sin(theta / 2),
            std::exp(cudaq::im<Scalar> * (phi + lambda)) * std::cos(theta / 2)};
  }
  case (GateName::PhasedRx): {
    ...
    return {{std::cos(phi / two), 0.},
            -cudaq::im<Scalar> * std::exp(-cudaq::im<Scalar> * lambda) *
