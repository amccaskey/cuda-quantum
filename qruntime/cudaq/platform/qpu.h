...
class QPU : public extension_point<QPU> {
protected:
...
CUDAQ_DEFINE_EXTENSION_IMPL(QPU)
#define CUDAQ_REGISTER_QPU(TYPE, NAME)                                         \
  static inline const std::string class_identifier = #NAME;                    \
  static std::unique_ptr<QPU> create() { return std::make_unique<TYPE>(); }
} // namespace cudaq
