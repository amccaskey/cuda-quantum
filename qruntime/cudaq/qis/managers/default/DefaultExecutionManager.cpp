...
class DefaultExecutionManager
    : public cudaq::BasicExecutionManager<DefaultExecutionManager> {

private:
  std::unique_ptr<cudaq::CircuitSimulator> simulator;
...
public:
  DefaultExecutionManager() {
    ...
    if (!cudaq::CircuitSimulator::is_registered(config.simulator))
      throw std::runtime_error(
          "[default execution_manager] invalid CircuitSimulator requested (" +
          std::string(config.simulator) + ")");
    simulator = cudaq::CircuitSimulator::get(config.simulator);
  ...
  static inline const std::string class_identifier = "default";
...
template <>
const bool cudaq::BasicExecutionManager<DefaultExecutionManager>::registered_ =
    cudaq::BasicExecutionManager<DefaultExecutionManager>::register_type();
