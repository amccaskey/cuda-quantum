...
public:
  ...
  basic_tensor<Rank, Scalar> operator*(Scalar other) const {
    // Create a tensor of all Scalar other elements
    basic_tensor<Rank, Scalar> otherT(
        std::vector<Scalar>(get_num_elements(), other), shape(), memory),
        result(shape(), memory);
    // Perform elementwise mult
    pimpl->elementwise_mult(otherT.pimpl.get(), result.pimpl.get());
    return result;
  }

  basic_tensor<Rank, Scalar>
  element_wise_mult(const basic_tensor<Rank, Scalar> &other)
    requires(Rank == 2)
  {
    if (shape() != other.shape()) {
      throw std::runtime_error(
          "basic_tensor shapes must match for elementwise mult");
    }
    basic_tensor<Rank, Scalar> result(shape(), memory);
    // Perform elementwise mult
    pimpl->elementwise_mult(other.pimpl.get(), result.pimpl.get());
    return result;
  }
  ...
  operator*(const basic_tensor<Rank, Scalar> &other) const {
    ...
    // If matrices, or matvec, perform dot
    // We want to retain natural C = A * B matmul or matvec
  ...
  ///
  /// @return Scalar* A pointer to the mutable data of the basic_tensor.
  ///
  ...
  /// data array.
  ///
  /// @return const Scalar * A const pointer to the immutable data of the
  /// basic_tensor.
  ///
  ...
  const Scalar *data() const { return pimpl->data(); }
