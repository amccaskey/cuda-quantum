...
public:
  void configure_qpus(std::vector<std::unique_ptr<qpu>> &platform_qpus,
    ...
    if (!isMqpu) {
      platform_qpus.emplace_back(qpu::get("circuit_simulator", metadata));
  ...
  CUDAQ_ADD_EXTENSION_CREATOR_FUNCTION(config::platform_config,
                                       simulator_config)
};
CUDAQ_REGISTER_EXTENSION(simulator_config)
} // namespace cudaq
