...
CUDAQ_INSTANTIATE_REGISTRY_NO_ARGS(cudaq::CircuitSimulator)
...
class local_simulator : public qpu_mixin<
...
public:
  local_simulator(const platform_metadata &metadata) : qpu_mixin(metadata) {
    // User may have provided custom options
    std::string simulatorName = "qpp";
    if (!metadata.options.empty()) {
      simulatorName = metadata.options.get<std::string>(
          std::vector<std::string>{"simulator"}, "qpp");
    }
