...
void RemoteSimulationState::execute() const {
  if (!state) {
    ...
    auto *as_mlir_launch = platform.as<v2::mlir_launch_trait>();
    if (!as_mlir_launch)
      throw std::runtime_error(
          "invalid qpu for remote simulation state execution.");
...
std::vector<std::complex<double>> RemoteSimulationState::getAmplitudes(
  ...
  // execute and then reset
  platform.set_execution_context(&context);
  platform.as<v2::mlir_launch_trait>()->launch_kernel(kernelName, args);
  platform.reset_execution_context();
...
RemoteSimulationState::overlap(const cudaq::SimulationState &other) {
  ...
  [[maybe_unused]] auto dynamicResult =
      platform.as<v2::mlir_launch_trait>()->launch_kernel(kernelName, nullptr,
                                                          nullptr, 0, 0, {});
  platform.reset_execution_context();
