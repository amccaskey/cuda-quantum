...
class StimCircuitSimulator : public CircuitSimulatorBase<double> {
protected:
...
public:
  ...
  CircuitSimulator *clone() override {
    thread_local static std::unique_ptr<CircuitSimulator> simulator =
        std::make_unique<StimCircuitSimulator>();
    return simulator.get();
  }
  CUDAQ_EXTENSION_CREATOR_FUNCTION_WITH_NAME(stim, CircuitSimulator,
                                             StimCircuitSimulator)
};

} // namespace cudaq

CUDAQ_REGISTER_EXTENSION_TYPE(cudaq::StimCircuitSimulator)
