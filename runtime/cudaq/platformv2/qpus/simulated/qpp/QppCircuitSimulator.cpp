...
class QppCircuitSimulator : public CircuitSimulatorBase<double> {
protected:
  /// The QPP state representation (qpp::ket or qpp::cmat)
  StateType state;
  // using StateType = qpp::ket;
...
public:
  ...
  CircuitSimulator *clone() override {
    thread_local static std::unique_ptr<CircuitSimulator> simulator =
        std::make_unique<QppCircuitSimulator>();
    return simulator.get();
  }
  CUDAQ_EXTENSION_CREATOR_FUNCTION_WITH_NAME(qpp, CircuitSimulator,
                                             QppCircuitSimulator<qpp::ket>)
};

#ifndef __CUDAQ_TOGGLE_QPPKET_REGISTRATION__
template <>
CUDAQ_REGISTER_EXTENSION_TYPE(QppCircuitSimulator<qpp::ket>)
#endif
