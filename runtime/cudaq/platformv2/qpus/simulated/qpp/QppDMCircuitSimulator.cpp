...
#define __CUDAQ_TOGGLE_QPPKET_REGISTRATION__
#include "QppCircuitSimulator.cpp"
#undef __CUDAQ_TOGGLE_QPPKET_REGISTRATION__
...
class QppNoiseCircuitSimulator : public QppCircuitSimulator<qpp::cmat> {
...
protected:
  ...
  void applyNoiseChannel(const std::string_view gateName,
    ...
    noise_model *localNoise = noise;

    // If noise not set, check the exec context
    if (!localNoise) {
      ...
      localNoise = const_cast<noise_model *>(executionContext->noiseModel);
    }
    ...
    auto krausChannels =
        localNoise->get_channels(gName, targets, controls, params);
...
public:
  ...
  CUDAQ_EXTENSION_CREATOR_FUNCTION_WITH_NAME(dm, CircuitSimulator,
                                             QppNoiseCircuitSimulator)
};

CUDAQ_REGISTER_EXTENSION_TYPE(QppNoiseCircuitSimulator)
