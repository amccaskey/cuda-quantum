...
class CuStateVecCircuitSimulator : public CircuitSimulatorBase<ScalarType> {
protected:
  ...
  using CircuitSimulatorBase<ScalarType>::tracker;
  using CircuitSimulatorBase<ScalarType>::nQubitsAllocated;
  using CircuitSimulatorBase<ScalarType>::stateDimension;
  using CircuitSimulatorBase<ScalarType>::calculateStateDim;
  using CircuitSimulatorBase<ScalarType>::executionContext;
  using CircuitSimulatorBase<ScalarType>::gateToString;
  using CircuitSimulatorBase<ScalarType>::x;
  using CircuitSimulatorBase<ScalarType>::flushGateQueue;
  using CircuitSimulatorBase<ScalarType>::previousStateDimension;
  using CircuitSimulatorBase<ScalarType>::shouldObserveFromSampling;
  using CircuitSimulatorBase<ScalarType>::summaryData;
  ...
  void addQubitToState() override {
    ...
    int dev;
    HANDLE_CUDA_ERROR(cudaGetDevice(&dev));
    cudaq::info("GPU {} Allocating new qubit array of size {}.", dev, 1);
    ...
    void *otherState;
    HANDLE_CUDA_ERROR(
        cudaMalloc((void **)&otherState, (1UL << 1) * sizeof(CudaDataType)));
    nvqir::initializeDeviceStateVector<CudaDataType>(
        n_blocks, threads_per_block, otherState, (1UL << 1));
    HANDLE_CUDA_ERROR(cudaGetLastError());

    {
      ...
      nvqir::kronprod<CudaDataType>(
          n_blocks, threads_per_block, previousStateDimension,
          deviceStateVector, (1UL << 1), otherState, newDeviceStateVector);
      HANDLE_CUDA_ERROR(cudaGetLastError());
    }
  ...
  void
  applyGate(const typename CircuitSimulatorBase<ScalarType>::GateApplicationTask
                &task) override {
...
public:
  ...
  void applyExpPauli(double theta, const std::vector<std::size_t> &controlIds,
    ...
    if (this->isInTracerMode()) {
      CircuitSimulator::applyExpPauli(theta, controlIds, qubits, term);
      return;
    }
    flushGateQueue();
  ...
  std::string name() const override { return "custatevec"; }
  CircuitSimulator *clone() override {
    thread_local static std::unique_ptr<CircuitSimulator> simulator =
        std::make_unique<CuStateVecCircuitSimulator<double>>();
    return simulator.get();
  }
};

class CuStateVecCircuitSimulatorF32 : public CuStateVecCircuitSimulator<float> {
public:
  CUDAQ_EXTENSION_CREATOR_FUNCTION_WITH_NAME(custatevec_fp32, CircuitSimulator,
                                             CuStateVecCircuitSimulatorF32)
};
CUDAQ_REGISTER_EXTENSION_TYPE(CuStateVecCircuitSimulatorF32);

class CuStateVecCircuitSimulatorF64
    : public CuStateVecCircuitSimulator<double> {
public:
  CUDAQ_EXTENSION_CREATOR_FUNCTION_WITH_NAME(custatevec_fp64, CircuitSimulator,
                                             CuStateVecCircuitSimulatorF64)
};

CUDAQ_REGISTER_EXTENSION_TYPE(CuStateVecCircuitSimulatorF64);
