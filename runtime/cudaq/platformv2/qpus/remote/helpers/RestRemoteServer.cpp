...
// AJM probably not the right way to do this...
...
static std::vector<std::unique_ptr<cudaq::v2::qpu_handle>> qpus;
    ...
    if (requestInfo.format == cudaq::CodeFormat::LLVM) {
      if (io_context.name == "sample") {
        ...
        if (!context.registerNames.empty()) {
          ...
          platform.set_execution_context(&io_context);
      ...
      if (io_context.name == "sample" &&
        ...
        auto *circuitSimulator =
            cudaq::v2::get_qpu().as<cudaq::v2::simulation_trait>();
        if (!circuitSimulator)
          throw std::runtime_error("Invalid qpu for remote simulation, must "
                                   "provide simulation_trait.");
...
protected:
  ...
  void *loadNvqirSimLib(const std::string &simulatorName) {
    qpus.clear();
    cudaq::v2::reset_override_qpu();
    auto config = cudaq::v2::config::platform_config::get("simulator_config");
    cudaq::config::TargetConfig cfg;
    config->configure_qpus(
        qpus, cudaq::v2::platform_metadata{
                  cfg, {"--target-option", "simulator:" + simulatorName}, ""});
    cudaq::v2::override_current_qpu(qpus[0].get());
    return nullptr;
  }
