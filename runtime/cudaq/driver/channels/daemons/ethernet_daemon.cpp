...
std::size_t malloc(std::size_t size) {
...
void send(std::size_t handle, std::vector<char> &data, std::size_t size) {
  auto *ptr = memory_pool.at(handle).first;
  std::memcpy(ptr, data.data(), size);
}

std::vector<char> recv(std::size_t handle, std::size_t size) {
...
#define RPCD_BIND_FUNCTION(NAME) srv->bind(#NAME, &cudaq::NAME);

int main(int argc, char **argv) {
  cudaq::info("Initialize Ethernet Daemon on Port {}", port);
