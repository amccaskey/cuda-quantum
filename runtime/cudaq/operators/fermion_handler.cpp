...
void normal_order_term(std::vector<std::size_t> &term,
                       details::scalar_parameter &coeff, fermion_data &data) {
  if (term.size() <= 2)
    return;
  ...
  for (std::size_t i = 0; i < ops.size() - 1; i++) {
    for (std::size_t j = i + 1; j < ops.size(); j++) {
      ...
      if (left.first == right.first && left.second == 0 && right.second == 1) {
        ...
        // For fermions: a a† = 1 - a†a
        data.productTerms.push_back({});
  ...
  // For fermions, check Pauli exclusion principle
  std::map<std::size_t, int> creation_counts;
  std::map<std::size_t, int> annihilation_counts;

  for (const auto &op : ops) {
    if (op.second == 1) {
      if (++creation_counts[op.first] > 1) {
        coeff = 0.0;
        term.clear();
        return;
      }
    } else {
      if (++annihilation_counts[op.first] > 1) {
        coeff = 0.0;
        term.clear();
        return;
      }
  ...
  // Apply final sign to coefficient
  coeff = coeff * sign;
...
fermion_data fermion_handler::initialize() const {
  return details::operator_data<term_t>{};
}

std::size_t fermion_handler::num_qubits(const fermion_data &data) const {
...
void fermion_handler::add_assign(fermion_data &thisPtr, const fermion_data &v) {
...
void fermion_handler::mult_assign(fermion_data &thisPtr,
                                  const fermion_data &v) {
...
bool fermion_handler::check_equality(const fermion_data &thisPtr,
                                     const fermion_data &v) const {
...
std::set<std::size_t>
fermion_handler::get_supports(const fermion_data &data) const {
...
fermion_handler::get_support_matrices(const fermion_data &data,
...
operator_matrix
fermion_handler::to_matrix(const fermion_data &data,
...
std::string fermion_handler::to_string(const fermion_data &thisPtr,
...
fermion_op create(std::size_t idx) {
  ...
  return fermion_op(std::move(data));
}
