...
details::operator_data spin_handler::initialize() const {
  return {{{0, 0}}, {1.}};
}
...
std::vector<double> spin_handler::get_data_representation(
    const details::operator_data &data) const {
...
void spin_handler::add_assign(details::operator_data &thisPtr,
...
void spin_handler::mult_assign(details::operator_data &thisPtr,
                               const details::operator_data &v) {
...
bool spin_handler::check_equality(const details::operator_data &thisPtr,
...
actionOnBra(const std::vector<std::size_t> &term, std::complex<double> coeff,
  ...
  for (std::size_t i = 0; i < nQ; i++) {
    if (term[i] && term[i + nQ]) {
      coeff *= (newConfiguration[i] == '1' ? i : -i);
      newConfiguration[i] = (newConfiguration[i] == '1' ? '0' : '1');
    } else if (term[i]) {
      newConfiguration[i] = newConfiguration[i] == '1' ? '0' : '1';

    } else if (term[i + nQ]) {
      coeff *= (newConfiguration[i] == '1' ? -1 : 1);
    }
  }
...
spin_handler::get_support_matrices(const details::operator_data &data,
                                   const parameter_map &p) const {
  return {};
}

operator_matrix spin_handler::to_matrix(const details::operator_data &data,
                                        const parameter_map &p) const {
  ...
  operator_matrix A(tmpData, {dim, dim});
  // auto rawData = A.data();
#if defined(_OPENMP)
#pragma omp parallel for shared(rawData)
  ...
  for (std::size_t rowIdx = 0; rowIdx < dim; rowIdx++) {
    ...
    for (std::size_t i = 0; auto &term : data.productTerms) {
      auto [res, coeff] =
          actionOnBra(term, data.coefficients[i++].constant_value(), rowBitStr);
      auto colIdx = std::stol(res, nullptr, 2);
      A[{convertOrdering(n, rowIdx), convertOrdering(n, colIdx)}] += coeff;
    }
  }
  return A;
}
