...
inline GateName gateNameFromString(const std::string &name) {
  static const std::unordered_map<std::string, GateName> nameMap = {
      {"x", GateName::X},      {"y", GateName::Y},
      {"z", GateName::Z},      {"h", GateName::H},
      {"s", GateName::S},      {"sdg", GateName::Sdg},
      {"t", GateName::T},      {"tdg", GateName::Tdg},
      {"rx", GateName::Rx},    {"ry", GateName::Ry},
      {"rz", GateName::Rz},    {"r1", GateName::R1},
      {"u1", GateName::U1},    {"u2", GateName::U2},
      {"u3", GateName::U3},    {"phased_rx", GateName::PhasedRx},
      {"swap", GateName::Swap}};
...
getGateByName(GateName name, const std::vector<Scalar> angles = {}) {
  ...
  case (GateName::U2): {
    ...
    return {{oneOverSqrt2, 0.},
            -oneOverSqrt2 * std::exp(lambda * im<Scalar>),
            oneOverSqrt2 * std::exp(im<Scalar> * phi),
            oneOverSqrt2 * std::exp(im<Scalar> * (phi + lambda))};
  }
  case (GateName::U3): {
    auto theta = angles[0];
    auto phi = angles[1];
    ...
    return {{std::cos(theta / 2), 0.},
            -std::exp(im<Scalar> * lambda) * std::sin(theta / 2),
            std::exp(im<Scalar> * phi) * std::sin(theta / 2),
            std::exp(im<Scalar> * (phi + lambda)) * std::cos(theta / 2)};
  }
  case (GateName::PhasedRx): {
    ...
    return {{std::cos(phi / two), 0.},
            -im<Scalar> * std::exp(-im<Scalar> * lambda) *
                std::complex<Scalar>{std::sin(phi / two), 0.},
            -im<Scalar> * std::exp(im<Scalar> * lambda) * std::sin(phi / two),
            std::cos(phi / two)};
  }
  case (GateName::Swap): {
    std::complex<Scalar> one = 1.;
    return {one, 0, 0, 0, 0, 0, one, 0, 0, one, 0, 0, 0, 0, 0, one};
  }
  }
...
struct swap {
  std::vector<ComplexT<ScalarType>>
  getGate(std::vector<ScalarType> angles = {}) {
    return getGateByName<ScalarType>(GateName::Swap);
  }
  const std::string name() const { return "swap"; }
};
