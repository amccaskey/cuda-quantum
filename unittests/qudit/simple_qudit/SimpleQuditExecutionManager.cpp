...
class SimpleQuditSimulator : public cudaq::v2::qpu<v2::simulation_trait> {
private:
  qpp::ket state;
...
public:
  ...
  void apply(const std::vector<std::complex<double>> &matrixRowMajor,
             ...
             const v2::operation_metadata &metadata) override {
  ...
  SimpleQuditSimulator(const v2::platform_metadata &m) : qpu(m) {}

  virtual ~SimpleQuditSimulator() = default;

  CUDAQ_EXTENSION_CUSTOM_CREATOR_FUNCTION(
      SimpleQuditSimulator,
      static std::unique_ptr<qpu> create(const v2::platform_metadata &m) {
        return std::make_unique<SimpleQuditSimulator>(m);
      })
};

CUDAQ_REGISTER_EXTENSION_TYPE(SimpleQuditSimulator)
