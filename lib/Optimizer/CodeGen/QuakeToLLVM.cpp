...
public:
  ...
  matchAndRewrite(quake::CustomUnitarySymbolOp op, OpAdaptor adaptor,
    ...
    auto paramsPtr = rewriter.create<LLVM::AllocaOp>(loc, LLVM::LLVMPointerType::get(rewriter.getF64Type()),
                                                     numParametersVal);
...
void cudaq::opt::populateQuakeToLLVMPatterns(LLVMTypeConverter &typeConverter,
  ...
  patterns.insert<AllocaOpRewrite, ConcatOpRewrite, CustomUnitaryOpRewrite,
                  DeallocOpRewrite, DiscriminateOpPattern,
                  ExtractQubitOpRewrite, ExpPauliRewrite,
