    ...
    if (auto ccCallableFunc =
      ...
      if (auto blockArg =
        ...
        auto replacement = module.lookupSymbol<func::FuncOp>(
            "__nvqpp__mlirgen__" + names[argIdx]);
        if (!replacement) {
          op.emitError("Invalid replacement function " + names[argIdx]);
          return failure();
        }
...
public:
  ...
  void runOnOperation() override {
    ...
    if (names.empty()) {
        // FIXME throw an error if we see there are 
        // callable arguments
        return;
    }

    patterns.insert<ReplaceCallIndirect>(ctx, names);
