...
public:
  ...
  LogicalResult matchAndRewrite(cudaq::cc::DeviceCallOp devcall,
    ...
    auto maybeNumBlocks = devcall.getNumBlocks();
    auto maybeNumThreadsPerBlock = devcall.getNumThreadsPerBlock();
    auto module = devcall->getParentOfType<ModuleOp>();
    auto devFuncName = devcall.getCallee();
    auto devFunc = module.lookupSymbol<func::FuncOp>(devFuncName);
    if (!devFunc) {
    ...
    // Note that for CUDA device kernels, we want the struct to be packed.
    auto structTy = cudaq::opt::factory::buildInvokeStructType(
        devFunc.getFunctionType(), 0, /*packed*/ maybeNumBlocks.has_value());
    ...
    genNewMarshalFunc(loc, marshalFunc, rewriter, callbackNameObj,
                      unmarshalFunc, structTy, devFunc, module, maybeNumBlocks,
                      maybeNumThreadsPerBlock);
  ...
  static void genNewMarshalFunc(Location loc, func::FuncOp marshalFunc,
                                ...
                                std::optional<std::size_t> maybeNumBlocks,
                                std::optional<std::size_t> maybeNumThreadsPerBlock) {
    ...
    dispatchArgs.push_back(rewriter.create<arith::ConstantIntOp>(
        loc, maybeNumBlocks.value_or(0), 64));
    dispatchArgs.push_back(rewriter.create<arith::ConstantIntOp>(
        loc, maybeNumThreadsPerBlock.value_or(0), 64));
