...
public:
  ...
  void decompose() {
    ...
    for (std::size_t j = 1; j <= numQubits; ++j) {
      ...
      auto target = j - 1;
      auto alphaYk = cudaq::compiler::getAlphaY(magnitudes, numQubits, k);
    ...
    for (std::size_t j = 1; j <= numQubits; ++j) {
      ...
      auto numControls = j - 1;
      auto target = j - 1;
      auto alphaZk = cudaq::compiler::getAlphaZ(phases, numQubits, k);
      if (alphaZk.empty())
        continue;
...
private:
  ...
  void applyRotation(const std::span<double> alphas, std::size_t numControls,
    ...
    auto thetas = cudaq::compiler::convertAngles(alphas);
    if (numControls == 0) {
      builder.applyRotationOp<Op>(thetas[0], qubitIndex(target));
      return;
    }

    auto controlIndices = cudaq::compiler::getControlIndices(numControls);
