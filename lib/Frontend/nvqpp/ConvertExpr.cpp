  ...
  if (isInNamespace(func, "cudaq")) {
    ...
    if (funcName == "detector") {
      if (args.size() == 1 && isa<cc::StdvecType>(args[0].getType())) {
        builder.create<cudaq::qec::DetectorVecOp>(loc, TypeRange{}, args[0]);
      } else {
        SmallVector<Value> measArgs;
        measArgs.reserve(args.size());
        for (auto v : args)
          measArgs.push_back(loadIfPtrToMeasure(v));
        builder.create<cudaq::qec::DetectorOp>(loc, TypeRange{}, measArgs);
      }
      return true;
    }
    if (funcName == "logical_observable") {
      builder.create<cudaq::qec::LogicalObservableOp>(loc, TypeRange{},
                                                     args[0]);
      return true;
    }
